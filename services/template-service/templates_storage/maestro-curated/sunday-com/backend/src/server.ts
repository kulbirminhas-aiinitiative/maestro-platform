...
io.use(async (socket, next) => {
  try {
    ...
    // const user = await validateWebSocketToken(token);
...
io.on('connection', (socket) => {
  Logger.websocket(`Client connected: ${socket.id}`);
  ...
  socket.on('subscribe', (data) => {
    ...
    if (channel) {
      socket.join(channel);
      Logger.websocket(`Client ${socket.id} subscribed to ${channel}`);

      socket.emit('subscribed', {
        channel,
        subscriptionId: `${socket.id}_${channel}`,
      });
    }
  });
  ...
  socket.on('unsubscribe', (data) => {
    ...
    if (channel) {
      ...
      Logger.websocket(`Client ${socket.id} unsubscribed from ${channel}`);
    }
  });
  ...
  socket.on('cursor_move', (data) => {
    ...
    if (boardId && cursor) {
      socket.to(`board:${boardId}`).emit('cursor_moved', {
  ...
  socket.on('disconnect', (reason) => {
    Logger.websocket(`Client disconnected: ${socket.id}, reason: ${reason}`);
  });
