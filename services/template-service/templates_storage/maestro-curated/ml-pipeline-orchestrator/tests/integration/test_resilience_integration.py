...
async def test_retry_with_timeout():
    """Retry and timeout should work together"""
    call_count = 0

    async def sometimes_slow_operation():
        nonlocal call_count
        call_count += 1
        if call_count < 2:
...
async def test_circuit_breaker_with_fallback():
    """Circuit breaker with fallback provides graceful degradation"""
    ...
    for _ in range(2):
        with pytest.raises(ValueError):
            await breaker.call(failing_primary)

    # Circuit is now open, fallback should handle it
    async def operation_with_breaker():
        return await breaker.call(failing_primary)
