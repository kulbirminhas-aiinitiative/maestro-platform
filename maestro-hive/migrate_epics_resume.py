...
def main():
    ...
    migrations = [
        {"old": "MD-2781", "new_name": "[Remediation] Core Services & CLI Compliance (Batch 2)", "desc": "Continued remediation of core services.", "target": "MD-2801"},
        {"old": "MD-2782", "new_name": "[Infrastructure] Service Mesh & Network Security (Batch 2)", "desc": "Continued infrastructure security work.", "target": None},
        {"old": "MD-2783", "new_name": "[Gateway] Kong API Gateway Implementation (Batch 2)", "desc": "Continued Gateway implementation.", "target": None},
        {"old": "MD-2784", "new_name": "[Workflow] Human-in-the-Loop Controls (Batch 2)", "desc": "Continued workflow controls.", "target": None},
        {"old": "MD-2785", "new_name": "[BAU] Operational Deployment & Compliance (Batch 2)", "desc": "Continued BAU operations.", "target": None}
    ]
    ...
    for mig in migrations:
        ...
        target_epic = mig.get('target')
        if not target_epic:
            target_epic = client.create_epic(mig['new_name'], mig['desc'])
        else:
            logger.info(f"Using existing target EPIC: {target_epic}")
            
        if target_epic:
            for ticket in tickets:
                client.set_parent(ticket['key'], target_epic)
                time.sleep(0.5)
