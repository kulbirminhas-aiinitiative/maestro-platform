...
# Quality Fabric - Testing as a Service (TaaS)
try:
    from quality_fabric_client import QualityFabricClient, PersonaType as QFPersonaType
    QUALITY_FABRIC_AVAILABLE = True
    logging.info("✅ Quality Fabric client loaded")
except ImportError as e:
    QUALITY_FABRIC_AVAILABLE = False
    QualityFabricClient = None
    QFPersonaType = None
    logging.warning(f"⚠️ Quality Fabric client not available: {e}")
...
try:
    from maestro_hive.teams.blueprint_scorer import BlueprintScorer
...
Respond with ONLY the JSON, no other text."""
        ...
        try:
            ...
            # MD-2054: Add warning when confidence below threshold
            CONFIDENCE_THRESHOLD = 0.8
            if classification.confidence_score < CONFIDENCE_THRESHOLD:
                logger.warning(f"  ⚠️  Low confidence ({classification.confidence_score:.0%} < {CONFIDENCE_THRESHOLD:.0%})")
                logger.warning(f"     Reason: Requirement may be ambiguous or too simple")
                logger.warning(f"     Suggestion: Consider providing more context or details")
            ...
            # MD-2054: Always warn on fallback since confidence is hardcoded at 60%
            logger.warning(f"  ⚠️  Low confidence ({fallback_classification.confidence_score:.0%}) - using heuristic fallback")
            logger.warning(f"     Claude SDK was unavailable or query failed")
...
class TeamExecutionEngineV2:
    ...
    async def execute(
        ...
        if QUALITY_FABRIC_AVAILABLE and execution_result.persona_results:
            logger.info("\n" + "="*80)
            logger.info("🔬 STEP 6: Quality Fabric Validation (TaaS)")
            logger.info("="*80)

            try:
                qf_client = QualityFabricClient()

                # Map persona IDs to Quality Fabric PersonaType
                persona_type_mapping = {
                    "system_architect": QFPersonaType.SYSTEM_ARCHITECT,
                    "backend_developer": QFPersonaType.BACKEND_DEVELOPER,
                    "frontend_developer": QFPersonaType.FRONTEND_DEVELOPER,
                    "database_engineer": QFPersonaType.DATABASE_ENGINEER,
                    "devops_engineer": QFPersonaType.DEVOPS_ENGINEER,
                    "qa_engineer": QFPersonaType.QA_ENGINEER,
                    "security_engineer": QFPersonaType.SECURITY_ENGINEER,
                    "technical_writer": QFPersonaType.TECHNICAL_WRITER,
                    "project_manager": QFPersonaType.PROJECT_MANAGER,
                    "product_manager": QFPersonaType.PRODUCT_MANAGER,
                    "ux_designer": QFPersonaType.UX_DESIGNER,
                }

                # Validate each persona's output
                for persona_id, persona_result in execution_result.persona_results.items():
                    # Determine persona type from ID
                    persona_type_key = persona_id.lower().replace("-", "_").replace(" ", "_")
                    qf_persona_type = persona_type_mapping.get(persona_type_key, QFPersonaType.BACKEND_DEVELOPER)
                    ...
                    # Call Quality Fabric
                    validation = await qf_client.validate_persona_output(
                        persona_id=persona_id,
                        persona_type=qf_persona_type,
                        output=output
                    )

                    qf_validations[persona_id] = {
                    ...
                    logger.info(f"   {persona_id}: {validation.status} ({validation.overall_score:.0f}%)")

                # Evaluate phase gate
                persona_results_for_gate = [
                    {"persona_id": pid, "overall_score": v["overall_score"]}
                    for pid, v in qf_validations.items()
                ]

                phase_gate = await qf_client.evaluate_phase_gate(
                    current_phase="implementation",
                    next_phase="testing",
                    phase_outputs={},
                    persona_results=persona_results_for_gate
                )

                qf_phase_gate = {
                    "status": phase_gate.get("status"),
                    "overall_quality_score": phase_gate.get("overall_quality_score"),
                    "gates_passed": phase_gate.get("gates_passed", []),
                    "gates_failed": phase_gate.get("gates_failed", []),
                    "blockers": phase_gate.get("blockers", []),
                    "bypass_available": phase_gate.get("bypass_available", False)
                }

                logger.info(f"\n   Phase Gate: {qf_phase_gate['status']} ({qf_phase_gate['overall_quality_score']:.0f}%)")

                # ========================================
                # PHASE GATE ENFORCEMENT (E2E Critical)
                # ========================================
                gate_status = qf_phase_gate.get("status", "unknown")

                if gate_status == "fail":
                    blockers = qf_phase_gate.get("blockers", [])
                    logger.error("="*80)
                    logger.error("❌ PHASE GATE BLOCKED - Quality threshold not met")
                    logger.error("="*80)
                    ...
                    return {
                        ...
                        "blockers": blockers,
                        "quality_score": qf_phase_gate.get("overall_quality_score", 0),
                        "recommendations": phase_gate.get("recommendations", []),
                        "error": "Phase gate blocked - quality threshold not met",
                        "quality_fabric": {
                            "available": True,
                            "persona_validations": qf_validations,
                            "phase_gate": qf_phase_gate
                        }
                    }

                elif gate_status == "warning":
                    logger.warning("="*80)
                    logger.warning("⚠️  PHASE GATE: CONDITIONAL PASS")
                    logger.warning("="*80)
                    warnings = qf_phase_gate.get("gates_failed", [])
                    for warning in warnings:
                        logger.warning(f"   WARNING: {warning}")
                    logger.warning("   Proceeding with warnings - consider addressing before deployment")
                    logger.warning("="*80)
                    # Continue execution but flag the warnings

                else:  # pass
                    logger.info("   ✅ Phase gate passed - proceeding to next phase")

            except Exception as e:
                logger.warning(f"⚠️ Quality Fabric validation failed: {e}")
                qf_validations = {"error": str(e)}
        else:
            logger.info("⚠️ Quality Fabric not available, skipping validation")
        ...
        result = {
            ...
            # Quality Fabric Validation (TaaS)
            "quality_fabric": {
                "available": QUALITY_FABRIC_AVAILABLE,
                "persona_validations": qf_validations,
                "phase_gate": qf_phase_gate
            } if qf_validations else None,
        ...
        # ============================================================
        # TRIMODAL VALIDATION (MD-2024: Enhanced Logging)
        # ============================================================
        # Correlation ID for tracking across all three validators
        correlation_id = f"trimodal-{session.session_id}"
        logger.info(f"\n{'='*60}")
        logger.info(f"🔺 TRIMODAL VALIDATION [correlation_id={correlation_id}]")
        logger.info(f"{'='*60}")
        ...
        if BDV_AVAILABLE:
            try:
                logger.info(f"[{correlation_id}] 🧪 Starting BDV validation...")
                bdv_service = get_bdv_integration_service()

                # Convert contracts for BDV validation
                # FIX MD-2021: Use original ContractSpecification objects which have
                # acceptance_criteria, not the simplified result['contracts'] dict
                contracts_for_bdv = []
                for contract in contracts:  # Use original contracts, not result['contracts']
                    ...
                    logger.debug(f"[{correlation_id}] BDV contract: {contract.name} with {len(contract.acceptance_criteria)} criteria")
                ...
                if contracts_for_bdv:
                    ...
                    logger.info(f"[{correlation_id}] ✅ BDV: {bdv_result.contracts_fulfilled}/{bdv_result.total_contracts} contracts, "
                               f"{bdv_result.scenarios_passed}/{bdv_result.total_scenarios} scenarios passed")
                else:
                    logger.warning(f"[{correlation_id}] ⚠️ BDV: No contracts to validate")
            except Exception as e:
                logger.error(f"[{correlation_id}] ❌ BDV validation failed: {e}")
                import traceback
                logger.debug(f"[{correlation_id}] BDV traceback: {traceback.format_exc()}")

        # ACC - Architectural Conformance Checking
        acc_metrics = None
        if ACC_AVAILABLE:
            try:
                logger.info(f"[{correlation_id}] 🏗️ Starting ACC validation...")
                acc_service = get_acc_integration_service()

                # Validate generated code architecture
                output_path = str(session.output_dir)
                logger.debug(f"[{correlation_id}] ACC analyzing: {output_path}")
                ...
                logger.info(f"[{correlation_id}] ✅ ACC: {status} (score: {acc_result.conformance_score:.2f}, "
                           f"violations: {acc_result.violations.total}, blocking: {acc_result.violations.blocking})")

                if acc_result.cycles_detected:
                    logger.warning(f"[{correlation_id}] ⚠️ ACC detected {len(acc_result.cycles_detected)} dependency cycles")
            except Exception as e:
                logger.error(f"[{correlation_id}] ❌ ACC validation failed: {e}")
                import traceback
                logger.debug(f"[{correlation_id}] ACC traceback: {traceback.format_exc()}")

        # Generate Tri-Modal Verdict
        if DDE_AVAILABLE:
            try:
                logger.info(f"[{correlation_id}] 📊 Generating verdict...")
                ...
                logger.debug(f"[{correlation_id}] DDE metrics: quality={dde_metrics['avg_quality_score']:.2f}, "
                            f"fulfillment={dde_metrics['contract_fulfillment_rate']:.2f}")
                ...
                logger.info(f"[{correlation_id}] ✅ VERDICT: Grade {verdict.grade.value} | Score {verdict.overall_score:.2f} | Decision: {verdict.deployment_decision.value}")
                logger.info(f"{'='*60}")
                logger.info(f"🔺 TRIMODAL VALIDATION COMPLETE [correlation_id={correlation_id}]")
                logger.info(f"{'='*60}\n")

                # ========================================
                # VERDICT ENFORCEMENT (E2E Critical)
                # ========================================
                from dde.verdict_aggregator import DeploymentDecision

                if verdict.deployment_decision == DeploymentDecision.REJECT:
                    logger.error("="*80)
                    logger.error("❌ DEPLOYMENT REJECTED - Quality/Compliance threshold not met")
                    logger.error("="*80)
                    logger.error(f"   Grade: {verdict.grade.value}")
                    logger.error(f"   Score: {verdict.overall_score:.2f}")
                    for finding in verdict.key_findings[:5]:  # Top 5 issues
                        logger.error(f"   FINDING: {finding}")
                    logger.error("   Action: Address critical issues before deployment")
                    logger.error("="*80)

                    # Update result to reflect rejection
                    result['success'] = False
                    result['deployment_rejected'] = True
                    result['rejection_reason'] = "Trimodal validation failed - deployment not recommended"

                elif verdict.deployment_decision == DeploymentDecision.CONDITIONAL:
                    logger.warning("="*80)
                    logger.warning("⚠️  DEPLOYMENT CONDITIONAL - Manual review required")
                    logger.warning("="*80)
                    logger.warning(f"   Grade: {verdict.grade.value}")
                    logger.warning(f"   Score: {verdict.overall_score:.2f}")
                    for finding in verdict.key_findings[:3]:
                        logger.warning(f"   FINDING: {finding}")
                    logger.warning("   Action: Review findings before deployment decision")
                    logger.warning("="*80)

                    result['deployment_conditional'] = True
                    result['requires_human_review'] = True

                else:  # APPROVE
                    logger.info(f"   ✅ Deployment approved - all quality gates passed")
