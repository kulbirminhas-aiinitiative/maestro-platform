...
def main():
    ...
    # Check MD-2781 to MD-2785
    epics = ["MD-2781", "MD-2782", "MD-2783", "MD-2784", "MD-2785"]
    jql = f'parent in ({",".join(epics)}) AND statusCategory != Done'
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Found {len(issues)} active tickets in target EPICs.")
            
            by_epic = {}
            for issue in issues:
                parent_key = issue['fields']['parent']['key']
                if parent_key not in by_epic: by_epic[parent_key] = []
                by_epic[parent_key].append(issue)
            
            for epic, items in by_epic.items():
                print(f"\nEPIC {epic}: {len(items)} tickets")
                for i in items:
